...
ps,:(`ld, 0b)	/ ld -> lock down variable 
...
scs:{
		...
		system("" sv ("touch "; d; "jobs"));
		system("" sv ("touch "; d; "tasks"));
...
lhs:{
		d: "" sv (system("echo $HOME"), "/q/hydrozoa_kb/"); s: "" sv (d; "ps");
		if["B"$ last system("" sv ("test ! -f "; s; "; echo $?")); 
			load `$s; s: "" sv (d; "jobs");
			if["B"$ last system("" sv ("test ! -f "; s; "; echo $?"));
				load `$s; s: "" sv (d; "tasks");
				if["B"$ last system("" sv ("test ! -f "; s; "; echo $?"));
